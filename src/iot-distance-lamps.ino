...
// Globals and code to handle button press
String changeEventName = "change_event";
bool pressing = false;
...
void setup() {
  // initializes Particle cloud functions
  Particle.function("next-color", nextColor);
  Particle.function("set-brightness", mySetBrightness);
  // set up passive pull-up
  pinMode(BUTTON, INPUT_PULLUP); // when button is pressed BUTTON_PIN will read LOW since it's connected to ground
  Serial.begin(9600);
  strip.begin();
...
void transition(Adafruit_NeoPixel* strip, uint32_t startingColor, uint32_t endingColor) {
  // Starting RGB values (NeoPixel color 32 bits format: WWWWWWWW RRRRRRRR GGGGGGGG BBBBBBBB)
